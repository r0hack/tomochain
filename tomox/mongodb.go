...
func (m *MongoDatabase) Commit() error {
	...
	for cacheKey, item := range db.pendingItems {
		r := &MongoItemRecord{
			Key:   cacheKey,
			Value: common.Bytes2Hex(item.Value.([]byte)), // item.Value is []byte
		}
		...
		log.Debug("Save", "cacheKey", cacheKey, "value", ToJSON(item.Value.([]byte)))
	}
...
func (m *MongoDatabase) CommitOrder(cacheKey string, o *Order) error {
